...
for i=1:numFrames
    ...
    grating(:,:,1) =   mean * ones(diameter) + amplitude*sin(2*pi* cyclePerPixel * R - phase*ones(diameter)) .* sin(2*pi*8/(2*pi)*T );
  
    %grating(:,:,1) =  mean * ones(diameter) + 0.25*sin(2*pi*8/(2*pi)*T );
 
    grating(:,:,2) = (R <= diameter/2); % alpha mask
    tex(i) = Screen('MakeTexture', window, grating);
end
...
moviePtr = Screen('CreateMovie',window,'circleoutslow.mov',diameter  ,diameter,60, ':CodecSettings=Videoquality=0.5 Profile=2');
...
while framecount < movieDurationFrames
    ...
    Screen('DrawTexture', window, tex(movieFrameIndices(framecount)), [],gratingRect, 0,0);
    ...
    Screen('AddFrameToMovie', window,gratingRect,'frontBuffer' );
