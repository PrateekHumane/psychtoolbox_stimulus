function scan = radialCheckers(comment, paradigmNumber)
...
try
    ...
    grating = zeros([size(distMatrix), 2]);
    grating(:,:,1) =  mean * ones(radius) + amplitude*sin(2*pi* cyclePerPixel * distMatrix ...
		- phase*ones(radius) );
    grating(:,:,2) = (distMatrix <= radius/2); % alpha mask
    ...
    fixRect = [centerX centerY centerX centerY]+...
        [-1 -1  1  1] * fixSize;
    ...
    while state~=conditionEnd
        ...
        %% Update state
        laststate = state;
        phase = mod(phase+ifi*gratingSpeed*pixPerDeg,2*pi); %deg/second * pix/deg
        grating(:,:,1) = mean * ones(radius) + amplitude*sin(2*pi* cyclePerPixel * distMatrix ...
            - phase*ones(radius) );
        gratingTexture = Screen('MakeTexture', windowPtr, grating);
